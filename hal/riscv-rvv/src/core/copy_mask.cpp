...
static int copyToMasked_e16c1(const uchar *src_data, size_t src_step, const uchar *mask_data, size_t mask_step,
                              uchar *dst_data, size_t dst_step, int width, int height) {
    for (; height--; mask_data += mask_step, src_data += src_step, dst_data += dst_step) {
        ...
        int vl;
        for (int i = 0; i < width; i += vl) {
            vl = __riscv_vsetvl_e16m8(width - i);
            auto m = __riscv_vmsne(__riscv_vle8_v_u8m4(mask_data + i, vl), 0, vl);
            auto src_v = __riscv_vle16_v_u16m8(src + i, vl);
            auto dst_v = __riscv_vle16_v_u16m8(dst + i, vl);
            __riscv_vse16_v_u16m8(dst + i, __riscv_vmerge(dst_v, src_v, m, vl), vl);
        }
    }
    return CV_HAL_ERROR_OK;
}
