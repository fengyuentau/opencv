...
static inline v_uint16 vx_not_zero(const v_uint16 &a)
{
    v_uint16 zero = vx_setzero_u16(), one = vx_setall_u16(1);
    return v_and(v_ne(a, zero), one);
}

static inline v_int16 vx_not_zero(const v_int16 &a)
{
    v_int16 zero = vx_setzero_s16(), one = vx_setall_s16(1);
    return v_and(v_ne(a, zero), one);
}

static inline v_uint32 vx_not_zero(const v_uint32 &a)
{
